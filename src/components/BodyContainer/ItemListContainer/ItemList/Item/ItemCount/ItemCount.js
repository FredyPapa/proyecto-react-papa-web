...
function ItemCount({ stock, handlerStock, handlerFinishBuy }) {
  //useState
  const [cantidad, setCantidad] = useState(0);
  const [, setStockProduct] = useState(stock);
  const [, setIsFinishBuy] = useState(false);
  //Aumentar cantidad
  ...
  const agregarCarrito = () => {
    if (cantidad > 0) {
      ...
      handlerStock(nuevoStock);
      //Actualizamos el estado para ocultar el componente
      handlerFinishBuy(true);
    }
  };
