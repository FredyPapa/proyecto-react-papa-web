...
function Item({ id, image, title, description, price, stock, titleNameCategory }) {
  ...
  const actualizarStock = (nuevoStock)=>{
    ...
    const nuevoItem = { id,title,price,cantidad,stock  }
    addItem(nuevoItem);
    //addItem({nuevoItem, quatntity:1});
  };
  //
    ...
    <div className="margenesContenido">
      <Card className="estiloCard">
        <Card.Header className="headerCardCategory">
          <small><strong>Categoría: </strong>{titleNameCategory}</small>
        </Card.Header>
